  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-12">
        ...
        {filledValues.map((v, i) => (
          ...
          >
            ...
            {v.statements.positive
              .filter((p) => p.trim())
              .map((p, j) => (
                <p key={j} className="mb-1.5 text-lg text-muted">
      ...
      {emptyValues.length > 0 && (
        <div className="mt-8">
          <h3 className="mb-3 text-lg font-medium text-muted">
          ...
          <div className="flex flex-wrap gap-2">
            {emptyValues.map((v, i) => (
              <span
                key={i}
                className="rounded-full border border-card-border bg-card-bg px-3 py-1.5 text-lg text-foreground"
              >
                {v.name}
              </span>
            ))}
